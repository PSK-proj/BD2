...
SELECT k.nazwa AS kwiaciarnia, g.nazwa AS gatunek, EXTRACT(MONTH FROM f.data_sprzedazy) AS miesiac, SUM(f.suma_pln) AS calkowita_sprzedaz
...
SELECT p.imie || ' ' || p.nazwisko AS pracownik, u.nazwa AS usluga, EXTRACT(MONTH FROM f.data_sprzedazy) AS miesiac, AVG(f.suma_pln) AS srednia_cena
FROM Fakt_Sprzedazy f
