...
SELECT k.nazwa AS kwiaciarnia,
  ...
  SUM(f.suma_pln * f.ilosc) OVER (PARTITION BY k.nazwa ORDER BY EXTRACT(MONTH FROM f.data_sprzedazy) ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) AS suma_sprzedazy
...
SELECT g.nazwa AS gatunek,
  EXTRACT(MONTH FROM f.data_sprzedazy) AS miesiac,
  AVG(f.suma_pln) OVER (PARTITION BY g.nazwa ORDER BY EXTRACT(MONTH FROM f.data_sprzedazy) RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS srednia_roczna,
  AVG(f.suma_pln) OVER (PARTITION BY g.nazwa ORDER BY EXTRACT(MONTH FROM f.data_sprzedazy)) AS srednia_miesieczna
FROM Fakt_Sprzedazy f
